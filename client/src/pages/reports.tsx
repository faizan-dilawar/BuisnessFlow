...
export default function Reports() {
  const [activeTab, setActiveTab] = useState<"pnl" | "transactions">("pnl");
  ...
  const { data: pnlData, isLoading, refetch } = useQuery({
    ...
    enabled: activeTab === "pnl" && !!dateRange.from && !!dateRange.to,
  });

  // Transactions Query
  const { data: transactions } = useQuery({
    queryKey: ["/api/reports/transaction", dateRange.from, dateRange.to],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/reports/transaction?from=${dateRange.from}&to=${dateRange.to}`);
      return res.json();
    },
    enabled: activeTab === "transactions" && !!dateRange.from && !!dateRange.to,
  });
    ...
    try {
      const endpoint =
        activeTab === "pnl"
          ...
          : `/api/reports/pnl/export?from=${dateRange.from}&to=${dateRange.to}&format=${format}&type=transactions`;
      ...
      link.download =
        activeTab === "pnl"
          ? `pnl-report.${format === "pdf" ? "pdf" : "xlsx"}`
          : `transactions-report.${format === "pdf" ? "pdf" : "xlsx"}`;
      link.click();
      link.remove();
    } catch (err) {
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
        ...
        <TabsContent value="pnl">
          {isLoading ? (
            ...
            <p className="text-center text-slate-600">No P&L data available.</p>
          )}
        </TabsContent>
                  ...
                  <tbody>
                    {transactions.map((t: any, i: number) => (
                      <tr key={i} className="border-b">
                        ...
                        <td className="p-2 border">{t.account}</td>
                        <td className="p-2 border">{formatCurrency(t.debit)}</td>
                        <td className="p-2 border">{formatCurrency(t.credit)}</td>
                        <td className="p-2 border">
